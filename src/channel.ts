...
): ChannelPlugin<ResolvedFeishuAccount> {
  return {
    id: "feishu",
    meta,
    ...
    status: {
      ...
      buildAccountSnapshot: ({ account, runtime: runtimeSnapshot, probe }) => {
        ...
        return {
          ...
          configured,
          tokenSource: account.tokenSource,
          running: runtimeSnapshot?.running ?? false,
          lastStartAt: runtimeSnapshot?.lastStartAt ?? null,
          lastStopAt: runtimeSnapshot?.lastStopAt ?? null,
          lastError: runtimeSnapshot?.lastError ?? null,
          probe,
          lastInboundAt: runtimeSnapshot?.lastInboundAt ?? null,
          lastOutboundAt: runtimeSnapshot?.lastOutboundAt ?? null,
        };
      },
      logSelfId: ({ account, runtime: runtimeEnv }) => {
        const appId = account.config.appId;
        if (appId) {
          runtimeEnv.log?.(`feishu:${appId}`);
        }
      },
    },
    gateway: {
